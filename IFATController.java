...
public class IFATController {
    private Hashtable<String, StudentIFATCard> studentCardList;

    public void loadCards(String folder){
        try {
            ...
            for (int i = 0; i < allFiles.length; i++) {
                ...
                StudentIFATCard card = new StudentIFATCard(filepath, allImages[i], cardMtx);
                // add the card to the hash table
                studentCardList.put(filepath, card);
            }
        } catch (Exception e) {
            // handle exceptions with a popup window here
        }
    }
    // method to display card image
    ...
    public int[][] detect(StudentIFATCard card){
        ...
        // create upper/lower bound Scalars for the grayscale
    ...
    public void exportData(StudentIFATCard card){
    ...
    private static int[][] mapToCard(int[][] mtx, int boxes, int questions) {
        ...
        for (int i = 0; i < boxPnts.length; i++) {
            ...
            if (boxPnts[i] != null) {
                ...
                System.out.println("(" + boxPnts[i].x + " ," + boxPnts[i].y + ") card Array(" + xPtr + " ," + yPtr +")");
